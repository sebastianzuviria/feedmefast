...
export async function deleteSite(siteId) {
    ...
    const feedbackRef = query(collection(db, 'feedback'), where('siteId', '==', siteId))
    const QuerySnapshot = getDocs(feedbackRef)
...
export async function updateSite(siteId, newValues) {
    const siteRef = doc(db, "sites", siteId)
    const updatedSite = await updateDoc(siteRef, newValues)
    return updatedSite
}
